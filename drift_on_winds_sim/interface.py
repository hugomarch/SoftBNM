...
def rotate_vector(v,theta):
    """ rotate a vector v from angle theta in degrees """
    theta *= 2*math.pi/360
    rot_mat = np.array([[math.cos(theta),-math.sin(theta)],[math.sin(theta),math.cos(theta)]])
    rot_v = np.dot(rot_mat,v)
    return rot_v

def x_y_interval_of_mesh_order(h,nb_of_mesh_node_in_wind_window_lat,mesh_order):
    """ return interval of wind mesh verticaly and horizontaly wise, in screen coordinates on the double world 
        interval is based on world height and number of nodes in mesh latitude wise """
    y_mesh_interval = h / (nb_of_mesh_node_in_wind_window_lat-1) / 2**mesh_order
...
def compute_wind_arrow(wind):
    wind_v = np.array([wind[0],-wind[1]])
    wind_arrow = wind_v * WIND_DRAWING_RATIO
    wind_dir = wind_v / np.linalg.norm(wind_v)
    # find coordinates of arrow head triangle
    arrow_head_point = [None]*4
    arrow_head_point[0] = wind_arrow + rotate_vector(wind_dir,90) * ARROW_HEAD_SIZE / 2
    arrow_head_point[1] = wind_arrow + rotate_vector(wind_dir,-90) * ARROW_HEAD_SIZE / 2
    arrow_head_point[2] = arrow_head_point[1] + rotate_vector(wind_dir,30) * ARROW_HEAD_SIZE
    arrow_head_point[3] = arrow_head_point[0]
    wind_arrow = {"body":[np.zeros(2),wind_arrow],"head":arrow_head_point}
    return wind_arrow

def get_wind_with_mesh_id(wind_at_mesh_id,day_wind_data,time,height,w,h,x_mesh_interval,y_mesh_interval,mesh_id):
    ...
    for lon_mesh_id, lat_mesh_id in mesh_id:
        x_node,y_node = x_mesh_interval * lon_mesh_id, y_mesh_interval * lat_mesh_id
        ...
        wind_arrow = compute_wind_arrow(wind)
        wind_at_mesh_id[(lon_mesh_id,lat_mesh_id)] = wind_arrow

def draw_wind_with_mesh_id(screen,wind_at_mesh_id,world_width,x_mesh_interval,y_mesh_interval,scale,x_screen,y_screen,mesh_id):
    """ draw wind at nodes listed in mesh_id on screen cut of double world.
        wind are retrieved in wind_at_mesh_id dictionary """
    for lon_mesh_id, lat_mesh_id in mesh_id:
        wind_arrow = wind_at_mesh_id[(lon_mesh_id,lat_mesh_id)]
        x_node,y_node = x_mesh_interval * lon_mesh_id, y_mesh_interval * lat_mesh_id
        for x_node_,y_node_ in [(x_node,y_node),(x_node+world_width,y_node)]:
            # coordinates on screen
            x_node_screen,y_node_screen = (x_node_-x_screen)*scale, (y_node_-y_screen)*scale
            # check that the node is indeed on screen (half is not)
            if 0 <= x_node_screen and x_node_screen <= W and 0 <= y_node_screen and y_node_screen <= H:
                node = np.array([x_node_screen,y_node_screen])
                arrow_body, arrow_head = wind_arrow["body"], wind_arrow["head"]
                pygame.draw.line(screen,(0,0,0),tuple(node+arrow_body[0]),tuple(node+arrow_body[1]))
                for point1,point2 in zip(arrow_head[:-1],arrow_head[1:]):
                    pygame.draw.line(screen,(0,0,0),tuple(node+point1),tuple(node+point2))
...
def main():
    ...
    nb_of_mesh_node_in_wind_window_lat = math.floor(180/LAT_INTERVAL/2) + 1
    ...
    while running:
        ...
        x_mesh_interval,y_mesh_interval = x_y_interval_of_mesh_order(world_height,nb_of_mesh_node_in_wind_window_lat,new_mesh_order)
        ...
        # list of nodes of the mesh that must be displayed
        displayed_node_mesh_id = []
        for di in range(nb_of_mesh_node_in_wind_window_lon):
            for dj in range(nb_of_mesh_node_in_wind_window_lat):
                node_mesh_id = ((new_screen_lon_mesh_id + di)%(max_lon_mesh_id+1), (new_screen_lat_mesh_id + dj)%(max_lat_mesh_id+1))
                displayed_node_mesh_id.append(node_mesh_id)
        ...
        mesh_id = []
        if redraw_all:
            mesh_id = displayed_node_mesh_id
        else:
            ...
            if redraw_min_lat_id is not None:
                for di in range(nb_of_mesh_node_in_wind_window_lon):
                    for lat_mesh_id in range(redraw_min_lat_id, redraw_max_lat_id+1):
                        lon_mesh_id = (new_screen_lon_mesh_id + di)%(max_lon_mesh_id+1)
                        node_mesh_id = (lon_mesh_id, (lat_mesh_id)%(max_lat_mesh_id+1))
                        mesh_id.append(node_mesh_id)
        # get wind at nodes in mesh_id 
        get_wind_with_mesh_id(wind_at_mesh_id,day_wind_data,time,height,world_width,world_height,x_mesh_interval,y_mesh_interval,mesh_id)
        ...
        screen_cut = double_world.subsurface((x_screen,y_screen), (int(W/scale),int(H/scale)))
        screen_cut = pygame.transform.scale(screen_cut, (W,H))
        # draw wind
        draw_wind_with_mesh_id(screen_cut,wind_at_mesh_id,world_width,x_mesh_interval,y_mesh_interval,scale,x_screen,y_screen,displayed_node_mesh_id)
